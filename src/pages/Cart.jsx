      ...
      {items.length === 0 ? (
        ...
        <>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {items.map((item, i) => (
              <div key={`${item.id}-${item.size}`} className="card lift-card fade-up cart-row" style={{ display: "flex", alignItems: "center", gap: 16, padding: 16, animationDelay: `${Math.min(i, 8) * 0.05}s` }}>
                <div className="cart-row-image" style={{ width: 64, height: 64, borderRadius: 8, overflow: "hidden", flexShrink: 0 }}>
                  <ProductImage image={item.image} accent={item.accent} size={30} />
                </div>
                <div className="cart-row-info" style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 500 }}>{item.name}</div>
                  <div className="muted" style={{ fontSize: 12 }}>{item.colorway} · Size {item.size}</div>
                </div>
                <div className="cart-row-qty" style={{ display: "flex", alignItems: "center", gap: 8 }}>
                ...
                <div className="cart-row-price" style={{ width: 70, textAlign: "right", color: "var(--accent)", fontWeight: 500, fontSize: 14 }}>${item.price * item.qty}</div>
                <button className="btn-icon cart-row-remove" style={{ width: 34, height: 34 }} onClick={() => removeFromCart(item.id, item.size)} aria-label="Remove"><TbTrash size={15} /></button>
              </div>
            ))}
          </div>

          <div className="card fade-up cart-summary" style={{ padding: 20, marginTop: 24, maxWidth: 360, marginLeft: "auto" }}>
