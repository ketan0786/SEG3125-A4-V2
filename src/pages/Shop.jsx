...
export default function Shop() {
  ...
  const [maxPrice, setMaxPrice] = useState(() => Number(params.get("maxPrice")) || 500);
  const [category, setCategory] = useState(() => params.get("category"));
  const [condition, setCondition] = useState(() => params.get("condition"));
  const [badgeFilter, setBadgeFilter] = useState(() => params.get("badge"));
  const [sort, setSort] = useState("Newest");
  const [query, setQuery] = useState(() => params.get("q") || "");

  // Nav links (SHOP/BRANDS/RELEASES/NEW IN/SALE) all point at this same /shop
  // route with different query params. React Router doesn't remount the page
  // when only the query string changes, so without this effect the filters
  // set on first mount would stay stuck if a different nav item is clicked
  // while already on this page. Re-sync from params whenever they change.
  useEffect(() => {
    setBrands(params.get("brand") ? [params.get("brand")] : []);
    setCategory(params.get("category"));
    setCondition(params.get("condition"));
    setBadgeFilter(params.get("badge"));
    setMaxPrice(Number(params.get("maxPrice")) || 500);
  ...
  function clearAll() {
    setBrands([]); setSize(null); setColor(null); setMaxPrice(500); setCategory(null); setCondition(null); setBadgeFilter(null);
  }
  ...
  const filtered = useMemo(() => {
    ...
    return list;
  }, [query, brands, size, color, maxPrice, category, condition, badgeFilter, sort]);
  ...
  const chips = [
    ...
    ...(badgeFilter ? [{ key: "badge", label: badgeFilter, clear: () => setBadgeFilter(null) }] : []),
  ];

  return (
    <div className="container fade-up" style={{ padding: "32px 32px 80px" }}>
      ...
      <div className="shop-layout" style={{ display: "grid", gridTemplateColumns: "200px 1fr", gap: 32, marginTop: 16 }}>
        {/* Sidebar facets */}
        <aside className="shop-sidebar" style={{ display: "flex", flexDirection: "column", gap: 28 }}>
          <div>
        ...
        <div>
          {filtered.length === 0 ? (
            ...
            <div className="grid-products" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16 }}>
              {filtered.map((p, i) => (
                <ProductCard key={p.id} product={p} subtitle={size ? `Size ${size}` : null} index={i} />
              ))}
            </div>
          )}
        </div>
